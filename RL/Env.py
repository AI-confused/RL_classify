...
class env_classfy(object):
    ...
    def reset(self):
        observation = np.array(json.loads(train_data_list[0][1]))
        self.done = 0
        self.point = 0
        # self.labels
        self.label = train_data_list[0][4]
        # print(self.labels)
    ...
    def step(self, action):
        ...
        # for _ in self.labels:
            # if _:
        if type_dic[self.label]/max_lable_count <= 0.1:
            type_minority_flag = 1
        elif type_dic[self.label]/max_lable_count > 0.1 and type_dic[self.label]/max_lable_count <= 0.5:
            type_half_flag = 1
        else:
            type_majority_flag = 1

        if buttom_action_space[action] == self.label:
            if type_minority_flag:reward = 1
            elif type_half_flag:reward = 0.5
            elif type_majority_flag:reward = 0.1
        else:
        ...
        s_ = np.array(json.loads(train_data_list[self.point][1]))
        # self.labels = []
        self.label = train_data_list[self.point][4]
        # print(self.labels)
        # for _ in contents[self.point]['type_id']:
        #     self.labels.append(''.join(list(_)[:4]))
        # print(self.labels)
